...
#include "execution/binding_id_iter/paths/all_shortest_simple/bfs_check.h"
#include "execution/binding_id_iter/paths/all_shortest_simple/bfs_enum.h"
#include "execution/binding_id_iter/paths/all_shortest_trails/bfs_check.h"
#include "execution/binding_id_iter/paths/all_shortest_trails/bfs_enum.h"
        ...
        for (auto& transition : automaton.from_to_connections[state]) {
            if (transition.inverse) {
                ...
                if (quad_model.use_default_index) {
                    if (path_semantic == PathSemantic::TRAILS ||
                        path_semantic == PathSemantic::ANY_TRAILS ||
    ...
    case PathSemantic::ACYCLIC : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AllSimple::BFSCheck<false>>(thread_info,
                                                                          path_var,
                                                                          from,
                                                                          to,
                                                                          automaton,
                    ...
                    return make_unique<Paths::AllSimple::DFSCheck<false>>(thread_info,
                                                                          path_var,
                                                                          from,
                                                                          to,
                                                                          automaton,
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AllSimple::BFSEnum<false>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         std::get<VarId>(to),
                                                                         automaton,
                    ...
                    return make_unique<Paths::AllSimple::DFSEnum<false>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         std::get<VarId>(to),
                                                                         automaton,
            ...
            if (to_assigned) {
                ...
                // TODO: allow no-nullable unfixed paths
                throw QuerySemanticException("property paths with ACYCLIC semantics must have at least 1 node fixed.");
            }
        }
    }
    case PathSemantic::SIMPLE : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AllSimple::BFSCheck<true>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         to,
                                                                         automaton,
                    ...
                    return make_unique<Paths::AllSimple::DFSCheck<true>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         to,
                                                                         automaton,
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AllSimple::BFSEnum<true>>(thread_info,
                                                                        path_var,
                                                                        from,
                                                                        std::get<VarId>(to),
                                                                        automaton,
                    ...
                    return make_unique<Paths::AllSimple::DFSEnum<true>>(thread_info,
                                                                        path_var,
                                                                        from,
                                                                        std::get<VarId>(to),
                                                                        automaton,
    ...
    case PathSemantic::ANY_ACYCLIC : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AnySimple::BFSCheck<false>>(thread_info,
                                                                          path_var,
                                                                          from,
                                                                          to,
                                                                          automaton,
                    ...
                    return make_unique<Paths::AnySimple::DFSCheck<false>>(thread_info,
                                                                          path_var,
                                                                          from,
                                                                          to,
                                                                          automaton,
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AnySimple::BFSEnum<false>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         std::get<VarId>(to),
                                                                         automaton,
                    ...
                    return make_unique<Paths::AnySimple::DFSEnum<false>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         std::get<VarId>(to),
                                                                         automaton,
            ...
            if (to_assigned) {
                ...
                // TODO: allow no-nullable unfixed paths
                throw QuerySemanticException("property paths with ANY_ACYCLIC semantics must have at least 1 node fixed.");
            }
        }
    }
    case PathSemantic::ANY_SIMPLE : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AnySimple::BFSCheck<true>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         to,
                                                                         automaton,
                    ...
                    return make_unique<Paths::AnySimple::DFSCheck<true>>(thread_info,
                                                                         path_var,
                                                                         from,
                                                                         to,
                                                                         automaton,
                ...
                auto provider = get_provider(automaton, thread_info);
                if (quad_model.path_mode == PathMode::BFS) {
                    return make_unique<Paths::AnySimple::BFSEnum<true>>(thread_info,
                                                                        path_var,
                                                                        from,
                                                                        std::get<VarId>(to),
                                                                        automaton,
                    ...
                    return make_unique<Paths::AnySimple::DFSEnum<true>>(thread_info,
                                                                        path_var,
                                                                        from,
                                                                        std::get<VarId>(to),
                                                                        automaton,
    ...
    case PathSemantic::ALL_SHORTEST_ACYCLIC : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                // bool case
                auto provider = get_provider(automaton, thread_info);
                return make_unique<Paths::AllShortestSimple::BFSCheck<false>>(thread_info,
                                                                              path_var,
                                                                              from,
                                                                              to,
                                                                              automaton,
                ...
                // enum starting on from
                auto provider = get_provider(automaton, thread_info);
                return make_unique<Paths::AllShortestSimple::BFSEnum<false>>(thread_info,
                                                                             path_var,
                                                                             from,
                                                                             std::get<VarId>(to),
                                                                             automaton,
            ...
            if (to_assigned) {
                ...
                // TODO: allow no-nullable unfixed paths
                throw QuerySemanticException("property paths with ALL_SHORTEST_ACYCLIC semantics must have at least 1 node fixed.");
            }
        }
    }
    case PathSemantic::ALL_SHORTEST_SIMPLE : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                // bool case
                auto provider = get_provider(automaton, thread_info);
                return make_unique<Paths::AllShortestSimple::BFSCheck<true>>(thread_info,
                                                                             path_var,
                                                                             from,
                                                                             to,
                                                                             automaton,
                ...
                // enum starting on from
                auto provider = get_provider(automaton, thread_info);
                return make_unique<Paths::AllShortestSimple::BFSEnum<true>>(thread_info,
                                                                            path_var,
                                                                            from,
                                                                            std::get<VarId>(to),
                                                                            automaton,
            ...
            if (to_assigned) {
                ...
                // TODO: allow no-nullable unfixed paths
                throw QuerySemanticException("property paths with ALL_SHORTEST_SIMPLE semantics must have at least 1 node fixed.");
            }
        }
    }
    case PathSemantic::ALL_SHORTEST_TRAILS : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                // bool case
                auto provider = get_provider(automaton, thread_info);
                return make_unique<Paths::AllShortestTrails::BFSCheck>(thread_info,
                                                                       path_var,
                                                                       from,
                                                                       to,
                                                                       automaton,
                ...
                // enum starting on from
                auto provider = get_provider(automaton, thread_info);
                return make_unique<Paths::AllShortestTrails::BFSEnum>(thread_info,
                                                                      path_var,
                                                                      from,
                                                                      std::get<VarId>(to),
                                                                      automaton,
            ...
            if (to_assigned) {
                ...
                // TODO: allow no-nullable unfixed paths
                throw QuerySemanticException("property paths with ALL_SHORTEST_TRAILS semantics must have at least 1 node fixed.");
            }
        }
    }
    }
