...
void BFSCheck<CYCLIC>::analyze(std::ostream& os, int indent) const {
    ...
    if (CYCLIC) {
        os << "Paths::AnySimple::BFSCheck<SIMPLE>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    } else {
        os << "Paths::AnySimple::BFSCheck<ACYCLIC>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    }
}
