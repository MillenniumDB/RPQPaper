...
void PathManager::print(std::ostream& os, uint64_t path_id) const {
    ...
    case ALL_SHORTEST_SIMPLE_MASK: {
        auto current_state = reinterpret_cast<const Paths::AllShortestSimple::PathState*>(paths[index][path_id & PATH_INDEX_MASK]);
        current_state->get_path(os);
        break;
    }
    ...
    case ALL_SHORTEST_TRAILS_MASK: {
        auto current_state = reinterpret_cast<const Paths::AllShortestTrails::PathState*>(paths[index][path_id & PATH_INDEX_MASK]);
        current_state->get_path(os);
        break;
    }
    default:
