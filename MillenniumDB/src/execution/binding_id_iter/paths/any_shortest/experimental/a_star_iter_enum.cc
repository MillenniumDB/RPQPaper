...
void AStarIterEnum::begin(BindingId& _parent_binding) {
    ...
    open.emplace(automaton.start_state,
                 start_object_id,
                 automaton.distance_to_final[automaton.start_state]);
...
bool AStarIterEnum::next() {
    if (is_first) {
        ...
        if (automaton.is_final_state[automaton.start_state]) {
            auto current_key = SearchState(automaton.start_state,
    ...
    while (open.size() > 0) {
        auto reached_state = expand_neighbors();
...
robin_hood::unordered_node_set<SearchState>::iterator AStarIterEnum::expand_neighbors() {
    auto current_state = &open.top();
