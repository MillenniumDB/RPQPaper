...
DijkstraEnum::DijkstraEnum(ThreadInfo*   thread_info,
                           VarId         path_var,
                           Id            start,
                           VarId         end,
...
bool DijkstraEnum::eval_data_check(uint64_t obj, vector<tuple<Operators, string, QueryElement>>& property_checks) {
...
void DijkstraEnum::begin(BindingId& _parent_binding) {
...
void DijkstraEnum::analyze(std::ostream& os, int indent) const {
    os << std::string(indent, ' ');
    os << "Paths::AnyShortest::DijkstraEnum(bpt_searches: " << bpt_searches
       << ", found: " << results_found <<")";
}
