            ...
            if (!is_simple_path(current_state.path_state, ObjectId(iter->get_node()))) {
                ...
                if (CYCLIC && automaton.is_final_state[transition.to]) {
                    ...
                    if (ObjectId(iter->get_node()) == start_object_id) {
                        auto node_reached_final = optimal_distances.find(iter->get_node());  // Only return shortest paths
                        if (node_reached_final != optimal_distances.end()) {
                            ...
                            optimal_distances.insert({iter->get_node(), current_state.distance + 1});
                            return visited.add(ObjectId(iter->get_node()),
                                               transition.type_id,
                                               transition.inverse,
                                               current_state.path_state);
...
void BFSEnum<CYCLIC>::analyze(std::ostream& os, int indent) const {
    ...
    if (CYCLIC) {
        os << "Paths::AllShortestSimple::BFSEnum<SIMPLE>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    } else {
        os << "Paths::AllShortestSimple::BFSEnum<ACYCLIC>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    }
}
