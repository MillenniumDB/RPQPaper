...
DFSCheck<CYCLIC>::DFSCheck(ThreadInfo* thread_info,
                           VarId       path_var,
                           Id          start,
                           Id          end,
                           RPQ_DFA     automaton,
...
template <bool CYCLIC>
void DFSCheck<CYCLIC>::begin(BindingId& _parent_binding) {
...
bool DFSCheck<CYCLIC>::next() {
    ...
    while (open.size() > 0) {
        ...
        if (CYCLIC) {
            // For SIMPLE discard states that contains the end node unless is the first state
            if (current_state.node_id == end_object_id && current_state.prev_state != nullptr) {
                stack<SearchStateDFS> empty;
                open.swap(empty);
                return false;
            }
        } else {
            // For ACYCLIC discard states that contains the end node
            if (current_state.node_id == end_object_id) {
                stack<SearchStateDFS> empty;
                open.swap(empty);
                return false;
...
SearchStateDFS* DFSCheck<CYCLIC>::expand_neighbors(SearchStateDFS& current_state) {
    ...
    while (current_state.transition < automaton.from_to_connections[current_state.automaton_state].size()) {
        ...
        while (current_state.iter->next()) {
            ...
            if (!is_simple_path(current_state, ObjectId(current_state.iter->get_node()))) {
                ...
                if (CYCLIC && automaton.is_final_state[transition.to]) {
                    ...
                    if (start_object_id == end_object_id && ObjectId(current_state.iter->get_node()) == start_object_id) {
                        return &open.emplace(
                            transition.to,
...
void DFSCheck<CYCLIC>::analyze(std::ostream& os, int indent) const {
    ...
    if (CYCLIC) {
        os << "Paths::AllSimple::DFSCheck<SIMPLE>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    } else {
        os << "Paths::AllSimple::DFSCheck<ACYCLIC>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    }
}


template class Paths::AllSimple::DFSCheck<true>;
template class Paths::AllSimple::DFSCheck<false>;
