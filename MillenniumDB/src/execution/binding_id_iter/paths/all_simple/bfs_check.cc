...
BFSCheck<CYCLIC>::BFSCheck(ThreadInfo* thread_info,
                           VarId       path_var,
                           Id          start,
                           Id          end,
                           RPQ_DFA     automaton,
...
template <bool CYCLIC>
void BFSCheck<CYCLIC>::begin(BindingId& _parent_binding) {
        ...
        while (iter->next()) {
            ...
            if (!is_simple_path(current_state.path_state, ObjectId(iter->get_node()))) {
                ...
                if (CYCLIC && automaton.is_final_state[transition.to]) {
                    ...
                    if (start_object_id == end_object_id && ObjectId(iter->get_node()) == start_object_id) {
            ...
            if (ObjectId(iter->get_node()) == end_object_id) {
...
void BFSCheck<CYCLIC>::analyze(std::ostream& os, int indent) const {
    ...
    if (CYCLIC) {
        os << "Paths::AllSimple::BFSCheck<SIMPLE>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    } else {
        os << "Paths::AllSimple::BFSCheck<ACYCLIC>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    }
}
