...
DFSEnum<CYCLIC>::DFSEnum(ThreadInfo* thread_info,
                         VarId       path_var,
                         Id          start,
                         VarId       end,
                         RPQ_DFA     automaton,
...
template <bool CYCLIC>
void DFSEnum<CYCLIC>::begin(BindingId& _parent_binding) {
        ...
        while (current_state.iter->next()) {
            ...
            if (!is_simple_path(current_state, ObjectId(current_state.iter->get_node()))) {
                ...
                if (CYCLIC && automaton.is_final_state[transition.to]) {
                    ObjectId start_object_id(std::holds_alternative<ObjectId>(start) ? std::get<ObjectId>(start)
                                                                                     : (*parent_binding)[std::get<VarId>(start)]);
                    if (ObjectId(current_state.iter->get_node()) == start_object_id) {
                        return &open.emplace(
                            transition.to,
...
void DFSEnum<CYCLIC>::analyze(std::ostream& os, int indent) const {
    ...
    if (CYCLIC) {
        os << "Paths::AllSimple::DFSEnum<SIMPLE>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    } else {
        os << "Paths::AllSimple::DFSEnum<ACYCLIC>(idx_searches: " << idx_searches << ", found: " << results_found << ")";
    }
}
